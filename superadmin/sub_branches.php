        ...
        if ($name && $admin_name && $admin_email && $admin_pass) {
            if (strlen($admin_pass) < 6) {
                ...
                try {
                    ...
                    $pdo->commit();
                    logActivity('company_created', "Created branch: $name with admin: $admin_email", $company_id);
                    $_SESSION['flash_message'] = "Sub-Branch '<strong>{$name}</strong>' created successfully!";
...
$companies = $pdo->query("
    ...
    WHERE c.is_main_branch = 0 GROUP BY c.id 
...
<head>
    ...
    <title>Sub-Branches - DRHrms</title>
...
<main class="main-content">
    ...
    <div class="page-header">
        <div>
            <h1>Sub-Branches (Sales Groups)</h1>
            <p style="color:var(--text-muted)">Create and assign sub-branches under a parent agency.</p>
        </div>
    ...
    <div class="content-card">
        <div class="card-header">
            <h2>All Sub-Branches (<?= count($companies) ?>)</h2>
        </div>
        <div style="overflow-x:auto">
            <table class="table">
                <thead>
                    <tr>
                        <th>Sub-Branch</th><th>Users</th><th>Status</th>
                        <th>Modules</th><th>Login Link</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($companies as $c): ?>
                    <?php $perms = $permissions[$c['id']] ?? []; ?>
                    <tr>
                        ...
                        <td style="color:var(--text-muted);font-size:0.85rem">
                            <?php
                                $slug = $c['login_slug'] ?? '';
                                $login_url = BASE_URL . 'login.php' . ($slug ? '?company=' . urlencode($slug) : '');
                            ?>
                            <?php if ($slug): ?>
...
<div class="modal-overlay" id="createModal">
    <div class="modal-box" style="max-width:650px;">
        ...
        <form method="POST">
            ...
            <div class="form-row">
                <input type="hidden" name="is_main_branch" value="0">
                <div class="form-group" id="parentBranchGroup">
