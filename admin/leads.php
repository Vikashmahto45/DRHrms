...
$perm->execute([$cid]);
if (!$perm->fetchColumn()) {
    die("<div style='padding:3rem;text-align:center;color:var(--text-main);background:#fff;min-height:100vh;font-family:sans-serif;'><h2>Module Locked</h2><p>The Lead CRM is not enabled for your account.</p><a href='dashboard.php' style='color:var(--primary-color)'>Back to Dashboard</a></div>");
}
        ...
        if ($row) {
            ...
            if ($can_edit && $old_status !== $new_status) {
                ...
                $pdo->prepare("INSERT INTO lead_history (lead_id, user_id, event_type, details) VALUES (?, ?, 'status_change', ?)")
                    ->execute([$lead_id, $_SESSION['user_id'], "Changed status from $old_status to $new_status"]);
...
";
$lead_stmt = $pdo->prepare($leads_query);
$params = [];
if ($source_filter) $params[] = $source_filter;
$lead_stmt->execute($params);
$leads_crm = $lead_stmt->fetchAll();
...
function getSourceBadge($source) {
    ...
    return "<span class='source-tag' style='background:".($color."22")."; color:$color; border:1px solid ".($color."44")."; font-weight:600; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem;'>$source</span>";
}
    ...
    <main class="main-content" style="margin-left: 0; width: 100%; padding: 2rem 3rem;">
        ...
        <div class="content-card">
            <div class="card-header">
                <h2>All Leads &amp; Walk-ins (<?= count($leads_crm) ?>)</h2>
            </div>
            <div style="overflow-x:auto">
                <table class="table">
                    ...
                    <tbody>
                            ...
                            <td>
                                <?php if ($role === 'admin' || $role === 'manager' || $uid == $l['assigned_to']): ?>
                                    <form method="POST">
                                        ...
                                        <select name="status" class="form-control" style="width:130px;padding:.4rem;font-size:.85rem;background:transparent;" onchange="this.form.submit()">
                                ...
                                <div style="display:flex; gap:0.4rem;">
                                    ...
                                    <?php if ($role === 'admin'): ?>
                                        <form method="POST" onsubmit="return confirm('Delete this lead forever?')">
                                            ...
                                            <input type="hidden" name="lead_id" value="<?= $l['id'] ?>">
                                            <button class="btn btn-sm btn-danger" style="background:none;border:none;cursor:pointer;">🗑️</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
                            </td>
                        ...
                        <?php endforeach; ?>
                        <?php if (!count($leads_crm)): ?>
                            <tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:2rem">No leads found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
