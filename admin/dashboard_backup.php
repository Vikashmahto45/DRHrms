...
$total_leads    = $pdo->prepare("SELECT COUNT(*) FROM leads WHERE company_id IN ($cids_in)");
...
$open_leads = $pdo->prepare("SELECT COUNT(*) FROM leads WHERE company_id IN ($cids_in) AND status='New'");
...
// Recent Leads (With Branch indicator logic if needed, but table fits strict columns)
$leads = $pdo->prepare("SELECT l.*, u.name AS assigned_name, c.name as company_name FROM leads l LEFT JOIN users u ON l.assigned_to=u.id LEFT JOIN companies c ON l.company_id = c.id WHERE l.company_id IN ($cids_in) ORDER BY l.created_at DESC LIMIT 5");
$leads->execute(); $leads = $leads->fetchAll();
...
// Simulated Revenue (Total leads * 500 for demonstration)
...
<div class="main-wrapper" style="flex: 1; margin-left: 260px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:1.5rem;margin-bottom:2rem;">
        <div class="content-card" style="margin-bottom:0;display:flex;align-items:center;gap:1rem;">
            ...
            <div>
                <div style="color:var(--text-muted);font-size:.85rem;">Total Leads</div>
            ...
            <div class="content-card" style="margin-bottom:0;">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <?php endforeach; ?>
                        <?php if (!count($leads)): ?><tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:1.5rem">No leads yet.</td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
            <?php endif; ?>
