...
try {
    ...
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN custom_sales_name VARCHAR(255) NULL AFTER verified_by");
    }

    $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'branch_id'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN branch_id INT NULL AFTER company_id");
    }

    $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'source'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN source VARCHAR(100) DEFAULT 'Walk-in' AFTER project_name");
    }

    $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'commission_percent'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN commission_percent DECIMAL(5,2) DEFAULT NULL AFTER total_value");
    }

    $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'start_date'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN start_date DATE NULL AFTER updated_at");
    }

    $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'end_date'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE projects ADD COLUMN end_date DATE NULL AFTER start_date");
    }
...
if (isset($_GET['verify']) && ($role === 'admin' || $role === 'manager')) {
    ...
    $pdo->prepare("UPDATE projects SET is_verified = 1, status = 'Active', verified_by = ? WHERE id = ? AND company_id = ?")
        ->execute([$uid, $pid, $cid]);
    header("Location: projects.php?msg=Verified");
    exit();
}
...
<head>
    ...
    <style>
        ...
        .st-Pending-Branch-Approval {
            background: rgba(99, 102, 241, 0.1);
            color: #6366f1;
        }

        .st-Pending-HQ-Review {
            background: rgba(99, 102, 241, 0.1);
            color: #6366f1;
        }
...
<body>
                ...
                <div style="display:flex;gap:10px;">
                    <?php if ($p_can_add): ?>
                        <button class="btn btn-primary"
                            onclick="document.getElementById('addProjectModal').classList.add('open')">+ New Project
                            Entry</button>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 1.5rem;">
                ...
                if ($is_hq) {
                    usort($projects, function ($a, $b) {
                        if ($a['status'] === 'Pending HQ Review' && $b['status'] !== 'Pending HQ Review')
                            return -1;
                        if ($a['status'] !== 'Pending HQ Review' && $b['status'] === 'Pending HQ Review')
                            return 1;
                        return 0;
                    ...
                    <div class="project-card">
                        <div
                            ...
                            <div>
                                ...
                                <div style="font-size:0.85rem; color:var(--text-muted); margin-top:4px;">Client:
                                    <strong><?= htmlspecialchars($p['client_name']) ?></strong></div>
                                <?php
                            ...
                            <div style="display:flex; gap:10px;">
                                <?php if ($p['status'] === 'Pending HQ Review' && $is_hq && in_array($role, ['admin', 'manager'])): ?>
                                    <a href="project_view.php?id=<?= $p['id'] ?>" class="btn btn-sm btn-primary"
                                        style="background:#ef4444; border:none;">Verify & Assign HQ Staff</a>
                                    <a href="project_view.php?id=<?= $p['id'] ?>#reject_section" class="btn btn-sm btn-outline"
                                        style="color:#ef4444; border-color:#ef4444;">Reject</a>
                                <?php else: ?>
                                    <a href="project_view.php?id=<?= $p['id'] ?>" class="btn btn-sm btn-primary">View
                                        Details</a>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    <div class="modal-overlay" id="addProjectModal">
        <div class="modal-box" style="max-width:500px;">
            ...
            <p style="font-size:0.85rem; color:var(--text-muted); margin-bottom:1.5rem;">Manual entry for walk-in or
                office project orders.</p>
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="project_name" id="project_service_select" class="form-control" required
                        ...
                        <?php foreach ($catalog as $svc): ?>
                            <option value="<?= htmlspecialchars($svc['name']) ?>"
                                data-comm="<?= $svc['commission_rate'] ?>"><?= htmlspecialchars($svc['name']) ?>
                                (<?= $svc['commission_rate'] ?>%)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group" style="flex:1;">
                        ...
                        <input type="number" step="0.01" name="commission_percent" id="modal_comm_pct"
                            class="form-control" style="background-color: #f1f5f9; cursor: not-allowed;" readonly
                            placeholder="Fixed by HQ">
                    </div>
                    <div class="form-group" style="flex:1;">
