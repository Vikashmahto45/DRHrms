...
// Check permission
$perm = $pdo->prepare("SELECT is_enabled FROM permissions_map WHERE company_id=? AND module_name='leads'");
$perm->execute([$cid]);
if (!$perm->fetchColumn()) {
    die("<div style='padding:3rem;text-align:center;color:var(--text-main);background:#fff;min-height:100vh;font-family:sans-serif;'><h2>Module Locked</h2><p>The Leads CRM is not enabled for your account.</p><a href='dashboard.php' style='color:var(--primary-color)'>Back to Dashboard</a></div>");
}
...
$usage_stmt = $pdo->prepare("SELECT COUNT(*) FROM leads WHERE company_id IN ($cids_in)");
$usage_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        if ($role !== 'admin') {
            die("Unauthorized: Only admins can create leads.");
        }
        ...
        if ($client) {
            if ($current_usage >= $lead_limit) {
                ...
                $pdo->prepare("INSERT INTO leads (company_id, client_name, phone, source, product, status, assigned_to, note) VALUES (?,?,?,?,?,?,?,?)")
                    ->execute([$cid, $client, $phone, $source, $product, 'New', $assign, $note]);
                logActivity('lead_captured', "New lead: $client for $product from $source", $cid);
    ...
    if ($action === 'update_status') {
        ...
        $stmt = $pdo->prepare("SELECT status, assigned_to FROM leads WHERE id=? AND company_id IN ($cids_in)");
        ...
        if ($row) {
            ...
            if ($can_edit && $old_status !== $new_status) {
                ...
                $pdo->prepare("UPDATE leads SET status=? WHERE id=? AND company_id IN ($cids_in)")
    ...
    if ($action === 'delete') {
        ...
        $pdo->prepare("DELETE FROM leads WHERE id=? AND company_id IN ($cids_in)")->execute([$id]);
...
<main class="main-content">
    <div class="page-header">
        <div>
            <h1>Lead CRM</h1>
            <p style="color:var(--text-muted)">Usage: <strong><?= $current_usage ?> / <?= $lead_limit ?></strong> Leads</p>
        </div>
    ...
    <!-- Leads Kanban / Table representation view -->
    <div class="content-card">
        <div class="card-header"><h2>All Leads &amp; Walk-ins (<?= count($leads) ?>)</h2></div>
        <div style="overflow-x:auto">
            <table class="table">
                ...
                <tbody>
                    ...
                    <?php endforeach; ?>
                    <?php if (!count($leads)): ?><tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:2rem">No leads yet. Create one manually or connect website forms.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
